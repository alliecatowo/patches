...
1. `/goal <objective>` creates a goal via `@prevalentware/opencode-goal-plugin` (see `opencode.json` / `tui.json`). Subsequent `/goal` without args reports status; `/goal pause|resume|clear|history|edit` manage it.
...
4. Execution: up to 40 concurrent top-level workers with disjoint file sets. Broad slices use a squad worker that may fan out up to four disjoint leaves at depth 2; every layer uses bounded ownership, concise handoffs, and `mise run check <ws>` via `bounded.sh`. No overlapping worktrees, no `git worktree add` by hand. Board moves via `projects_write`/`issue_write` + `Fixes #N` merges (see `AGENTS.md:29`).

MCPs available to every agent: `github` (remote, `projects`+`issues` toolsets, needs `GITHUB_PAT`) and `mise` (stdio, `mise mcp`) — both declared in `opencode.json` so they're repo-portable, plus any repo-level `.mcp.json` entries. Add a new MCP by adding a 5-line entry there; don't scatter per-agent configs.
...
- `opencode.json` — ceilings, free fallbacks, and `subagent_depth`

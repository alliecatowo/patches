...
| --------------------------------------- | ---------------------------- | --------------- | ------------------------------------------------------------------------ | ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **goal-driver**                         | `grok-4-1-fast-reasoning`    | 800k            | $0.20 / $0.50 / $0.05                                                    | 2M window                       | **reasoning: high** — persistent `/goal` entrypoint after planning exists. Finesse at luna price. Extra reasoning pays for boring routing + queue-first triage. Never does ticket impl. Fallback: `grok-4-3` 180k or `luna` 150k.                                                                                                     |
| **worker** (default, ~90% of tokens)    | `deepseek-v4-flash`          | 140k            | $0.22 off-peak / $0.44 peak in, $0.66/$1.32 out, $0.007/$0.014 cache hit | —                               | **reasoning: medium-low** — bounded low-ambiguity impl. Light extra reasoning goes a long way on explicit packets; too much on mechanical confuses it. Fallback chain: `deepseek-v4-flash` → `opencode/muse-spark-1.2-contributor-free` → `opencode/nemotron-3-ultra-free` → `qwen3.7-flash` ($0.03). Exhaust free before paid retry. |
| **mid-worker** (capability retry)       | `deepseek-v4-pro`            | 140k            | $0.55 / $1.80 / $0.014                                                   | —                               | **reasoning: medium** — stronger than flash, far cheaper than terra (10×). Used when flash+free failed but work is still bounded impl, not integration. Fallback: `nemotron-3-ultra-free` → `qwen3.7-flash` ($0.03) before terra. Ladder now `deepseek → free → pro/qwen → terra → grok`.                                             |
| **researcher / docs-writer / verifier** | `qwen3.7-flash` / `deepseek` | 120k            | $0.03 / $0.13 / $0.006                                                   | —                               | **reasoning: low** — docs lookup, truthful docs, scoped checks. Mechanical, keep lean. Always `WebSearch/WebFetch` before guessing.                                                                                                                                                                                                   |
...
| **architect / auditor**                 | `grok-4-6`                   | 180k            | $2.00 / $6 / $0.50                                                       | **200k → $4/$12 whole-request** | **reasoning: high** — design, ADRs, major decompositions, milestone audits. Fresh session per invocation. Cheaper fallbacks: `grok-4-3` (1M, $1.25/$2.50) or `grok-4-1-fast-reasoning` ($0.20/$0.50, 2M) when 1M-doc or cost matters.                                                                                                 |
...
Why these ceilings: Grok-4.6/4.3 double at 200k, GPT-5.6 at 272k — the higher rate prices the _whole_ request. Grok-4.1-fast is 2M, driver pinned at 800k for headroom. Fork/compact at 175-180k (Grok) and ~150k/220k (Luna/Terra) to stay below. DeepSeek peak is 01-04 + 06-10 UTC; off-peak is half — schedule batch work outside those 7h when possible.

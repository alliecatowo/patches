...
| `BeginDeviceLink`            | planned     | ADR 0037 §1. New device posts a device-signed link offer (never root-signed). The node stores it opaquely for at most 10 minutes, at most 3 pending per actor, and never decodes it on the authority device's behalf.                                                                                                             |
| `ListPendingDeviceLinks`     | planned     | ADR 0037 §1. The caller's own pending offers only, for the authority device to decode, re-verify, and derive the SAS from the bytes it received — never the node's decoded view.                                                                                                                                                  |
| `CancelDeviceLink`           | planned     | ADR 0037 §1, §3.4. Discards a pending offer before consumption or expiry; an offer is never reused once cancelled, consumed, or expired.                                                                                                                                                                                          |
...
| `ListMailboxEnvelopes`       | implemented | Keyset on `(received_at, envelope_id)` ascending. Poll-based; no push, no stream, no sort parameter. Optional `conversation_id` filter (issue #152): applied server-side before paging, so an open thread's 5 s poll no longer walks and discards every other conversation's queued mail. Unset preserves whole-mailbox behavior. |

...
| 5. Sesame fanout / two-device interop           | PARTIAL  | Own-device fanout is implemented (`apps/server/src/modules/e2ee/e2ee-fanout.ts`). `apps/tui/src/e2ee/two-device-interop.test.ts` (#273a) covers device-link end to end (SAS, roster convergence, rollback rejection) and reproduces, as a failing-closed assertion, why a live post-link fanout send still can't complete: no TUI API re-signs a device's prekey bundle (or revokes a single device) after a roster change.                                             |

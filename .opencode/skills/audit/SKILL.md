...
Don't run this more than once per phase boundary — it's a `grok-4-6` 180k audit, not a cheap check; use `mise run check` / normal review for routine work. `milestone-auditor` is the scoped variant for a single milestone gate.

...
2. **Board triage:** `projects_list`/`projects_get` → ready Todo ∩ not Blocked → disjoint file sets. One PR/issue per worker, never duplicate assignment.
3. **Dispatch ladder:** `deepseek-v4-flash` first; free `opencode/*` second; `gpt-5.6-terra` only on `blocker=capability` after one cheap failure (see `triage/SKILL.md`). `terra` never on first delegation.
4. **Packet:** ≤15 lines per `packet/SKILL.md`, scoped validation only (`mise run check <ws>`).

...
Ladder: `deepseek-v4-flash` (leaf, 90% tokens) → `opencode/*-free` (free retry) → `deepseek-v4-pro`/`qwen3.7-flash` (mid, ~$0.55/$1.80 vs terra $2/$12 — burn cheap before terra) → `gpt-5.6-terra` (senior, only on `capability` after mid) → `grok-4-6`/`grok-4-3` (architect replan). `gpt-5.6-sol` only on explicit `escalate: sol` — it's PREMIUM and 20x Luna. See `execution-loop/SKILL.md` queue-first ordering.

Classify first:

- **Env** (port contention, flock, DB down, /tmp inodes, bounded.sh wait, missing .env): retry same model, same packet. Do not escalate.
- **Capability** (model couldn't reason, hit context ceiling, flaky tool use): retry with next rung (`deepseek → free → pro/qwen → terra`). One rung at a time; two identical failures → change approach or replan. Never jump `deepseek → terra` — use mid.

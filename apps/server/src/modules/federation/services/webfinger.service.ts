...
export class WebfingerService {
  ...
  async resolve(resource: string): Promise<{ subject: string; links: unknown[] } | undefined> {
    const match = RESOURCE_PATTERN.exec(resource);
    if (match === null) return undefined;
    const [, handle, domain] = match;
    if (handle === undefined || domain === undefined) return undefined;
    if (domain.toLowerCase() !== this.nodeHost()) return undefined;
    ...
    return buildWebfingerJrd(`${actor.handleNormalized}@${domain.toLowerCase()}`, actorUri);
  }

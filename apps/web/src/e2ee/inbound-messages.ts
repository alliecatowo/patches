 ...
 * persisted the sender's side): before this store a received message existed only in the
 * session-scoped drain cache and vanished on reload or restart. These rows live under the
 * same authenticated encryption as ratchet state, one reserved opaque record per
 * conversation (the leading NUL cannot occur in a real session id, matching
 * `ENROLLMENT_RECORD_KEY`/`PEER_PIN_RECORD_KEY`), keyed by the delivering envelope id so a
 * lost-acknowledgement replay is never rendered twice. They are erased by `vault.wipe()`

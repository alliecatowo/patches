 ...
 * hand-rolled bytes. Web port of `apps/tui/test/e2ee-chain.test.ts`.
 */
...
import {
  E2eeDeviceCertificateSchema,
  E2eeDeviceRosterSchema,
  E2eeDeviceStatus,
  E2eeIdentityRootSchema,
  E2eeRosterEntrySchema,
...
function buildChain(actorId: string, deviceId: string, rootSeedText: string): ChainFixture {
  ...
  const certificatesWire: E2eeDeviceCertificate[] = [
    create(E2eeDeviceCertificateSchema, {
      actorId,
      deviceId,
      rootGeneration: 1,
      certificateVersion: E2EE_DEVICE_CERTIFICATE_VERSION,
  ...
  const rosterWire: E2eeDeviceRoster = create(E2eeDeviceRosterSchema, {
    actorId,
    sequence: 1n,
    rootGeneration: 1,
    previousDigest: new Uint8Array(32),
  ...
  const rootWire: E2eeIdentityRoot = create(E2eeIdentityRootSchema, {
    actorId,
    generation: 1,
...
describe('verifyActorChain — transcript/root-key and timestamp binding (M4)', () => {
  ...
  it('rejects a served roster whose wire createdAt disagrees with its signed transcript', () => {
    ...
    const tamperedRoster: E2eeDeviceRoster = create(E2eeDeviceRosterSchema, {
      ...fix.rosterWire,

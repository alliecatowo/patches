 ...
 * web port of the TUI's `group-control.ts` (issue #168).
 *
...
import { E2eeGroupChangeKind, type E2eeGroupControlEvent } from '@patches/proto/es';
...
function changeOf(wire: E2eeGroupControlEvent): 'ADDED' | 'REMOVED' | undefined {
  if (wire.change === E2eeGroupChangeKind.ADDED) return 'ADDED';
  if (wire.change === E2eeGroupChangeKind.REMOVED) return 'REMOVED';
  return undefined;
}

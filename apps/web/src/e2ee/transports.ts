...
export function bindConversationCreate(api: E2eeApiSurface): E2eeConversationCreateTransport {
  return {
    ...
    // exist yet when the client sealed it. Composing that follow-up `SendEnvelopes`
    // (via the ordinary `send()` path, now that this id exists) is the caller's job,

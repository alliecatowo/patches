/**
 * The optional recovery archive — sealing and restore composition (issue #272, ADR 0020
 * §10). Byte-for-byte port of `apps/tui/src/e2ee/recovery-archive.ts`: every function
 * here is pure `@patches/crypto`/`@patches/domain` composition with no Node-specific API,
 * so the browser build is identical logic, not a re-implementation to keep in sync by
 * hand. If the TUI seam changes, mirror the change here (ADR 0034 Stage 1: the two
 * clients' vault-adjacent seams stay in lockstep).
 *
 ...
 * the same bytes. This module is the browser half that needs real crypto:
 *
 ...
 * through `EnrollDevice`/`PublishDeviceRoster`; the plan this module returns is the only
 * input it needs. The archive carries no live counters, skipped keys, prekeys, or device
 * identity keys to restore even if someone tried.
 *

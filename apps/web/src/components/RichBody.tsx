...
function RichBodyImpl({ source }: RichBodyProps): JSX.Element {
  // P301: Memoize markup AST parsing to prevent re-parsing on parent component re-renders
  const blocks = useMemo(() => parseMarkup(source), [source]);
  return <>{blocks.map((block, index) => renderBlock(block, index))}</>;
}

// Prevents re-rendering RichBody when parent PostCard or timeline re-renders with unchanged source

    ...
    return (
      <div className={styles['scroller']} role="log" aria-label="Messages" aria-live="polite">
        <div className={styles['emptyThread']}>
          <p>{emptyLabel ?? 'No messages yet.'}</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`${styles['group']} ${group.mine ? styles['groupMine'] : ''}`}>
      ...
      {group.rows.map((row) =>
        row.kind === 'message' ? (
          <p
            key={row.id}
            className={`${styles['bubble']} ${group.mine ? styles['mine'] : styles['theirs']}`}
          >
            {row.body}
          </p>
        ) : null,
...
function NonMessageRow({ row }: { row: InboxRow }): JSX.Element {
  if (row.kind === 'unverifiable') {
    return (
      <div className={styles['emptyThread']} style={{ margin: '0.4rem auto' }}>
  ...
  if (row.kind === 'history') {
    return (
      <div style={{ padding: '0.5rem 0', color: 'var(--fg-muted)' }}>

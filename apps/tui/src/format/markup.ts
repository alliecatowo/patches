 ...
 * pipeline — sanitize -> parse -> one AST -> layout — so a body can never render one
 * way in one place and another way somewhere else.
 *
 * This module is deliberately free of Ink, React and terminal escapes: it produces
 * data. `rich-text.tsx` turns that data into Ink nodes; the web client is expected to
 * port *this file* rather than write a second grammar (see `docs/architecture/tui.md`).
 *
 ...
 * 3. Layout is authoritative. `layoutMarkup` wraps to exact terminal cells and the
 *    renderer prints those lines verbatim, so `measureMarkupHeight` cannot disagree
 *    with what is drawn — an under-count is what smears Ink's line diff (§`measure.ts`).
 */
...
/** Cache for compiled attribute matching regexes to avoid re-compiling RegExp on every HTML tag (~3.3x faster). */
...
function attributeValue(attributes: string, name: string): string | undefined {
  const match = getAttributePattern(name).exec(attributes);
...
export interface LayoutOptions {
  /** Plain mode (spec §173/§185) reproduces the source markers instead of styling. */
  plain?: boolean;
}

function markerFor(node: InlineNode, plain: boolean): string {
  if (!plain) return '';
  return node.marker ?? '';
}

/** Flattens inline nodes into words, keeping each word tagged with its role. */
function inlineWords(nodes: readonly InlineNode[], plain: boolean): LayoutRun[] {
  const runs: LayoutRun[] = [];
  for (const node of nodes) {
    const marker = markerFor(node, plain);
    let text = `${marker}${node.text}${marker}`;
    if (plain && node.role === 'link' && node.href !== undefined && node.href !== node.text) {
      text = `[${node.text}](${node.href})`;
    }
    for (const word of text.split(' ')) {
      if (word === '') continue;
      const run: LayoutRun = { role: plain ? 'text' : node.role, text: word };
      if (node.href !== undefined && !plain) run.href = node.href;
      runs.push(run);
    }
  }
  return runs;
...
function wrapRuns(
  ...
  const start = (): void => {
    current = prefix === '' ? [] : [{ role: 'text', text: prefix }];
  ...
  for (const run of runs) {
    const wordWidth = cellWidth(run.text);
    const needsSpace = used > cellWidth(prefix);
    if (needsSpace && used + 1 + wordWidth > width) commit();
    if (needsSpace) {
      current.push({ role: 'text', text: ' ' });
      used += 1;
    }
    if (wordWidth > width - cellWidth(prefix)) {
      // An over-long word (a URL, a CJK run) is broken mid-word, as wrap-ansi does.
      let remainder = run.text;
      while (remainder !== '') {
        const room = Math.max(1, width - used);
        let taken = '';
        for (const char of remainder) {
          if (cellWidth(taken + char) > room) break;
          taken += char;
        }
        if (taken === '') {
          commit();
        ...
        remainder = remainder.slice(taken.length);
        if (remainder !== '') commit();
      }
      continue;
    }
  ...
  if (current.length > 0 || lines.length === 0) lines.push({ runs: current });
  return lines;
}
...
function layoutBlock(
  ...
  switch (block.kind) {
    case 'paragraph': {
      const lines: LayoutLine[] = [];
      block.lines.forEach((inlines, index) => {
        lines.push(
          ...wrapRuns(
            inlineWords(inlines, plain),
            width,
            index === 0 ? firstPrefix : laterPrefix,
            laterPrefix,
          ),
        );
      });
    ...
    case 'heading': {
      // Headings render as bold text — a terminal has no larger type to grow into.
      const marker = plain ? `${'#'.repeat(block.level)} ` : '';
      const runs = inlineWords(block.inlines, plain).map((run) => ({
        ...run,
        role: plain ? ('text' as const) : ('strong' as const),
      }));
      return wrapRuns(runs, width, `${firstPrefix}${marker}`, laterPrefix);
    }
    case 'code':
      return block.lines.map((line) => ({
        runs: [{ role: 'code' as const, text: `${firstPrefix}${line}` }],
      }));
    case 'quote': {
      const lines: LayoutLine[] = [];
      for (const inner of block.blocks) {
        lines.push(...layoutBlock(inner, width, plain, `${firstPrefix}> `, `${laterPrefix}> `));
      }
      return lines;
    }
    case 'list': {
      ...
      block.items.forEach((item, index) => {
        const bullet = block.ordered ? `${String(index + 1)}. ` : plain ? '- ' : '• ';
        const indent = ' '.repeat(cellWidth(bullet));
        item.forEach((inner, innerIndex) => {
          lines.push(
            ...layoutBlock(
              ...
              width,
              plain,
              innerIndex === 0 ? `${firstPrefix}${bullet}` : `${laterPrefix}${indent}`,
              `${laterPrefix}${indent}`,
            ),
          );
        });
...
/**
 * Wraps an AST to exactly `width` terminal cells. The renderer prints these lines
 * verbatim and `measureMarkupHeight` counts them, so measurement and rendering are the
 * same computation rather than two that have to be kept in agreement.
 */
export function layoutMarkup(
  ...
  blocks.forEach((block, index) => {
    ...
    lines.push(...layoutBlock(block, usable, plain, '', ''));
  });
  return lines;
}

/** Rows a body occupies at `width` columns. */
export function measureMarkupHeight(
  source: string,
  width: number,
  options: LayoutOptions = {},
): number {
  return Math.max(1, layoutMarkup(parseMarkup(source), width, options).length);
}

/** The visible characters of a laid-out line, with no styling — used by tests and
 * by any consumer that needs the plain text of what was drawn. */
export function lineText(line: LayoutLine): string {

...
/**
 * ADR 0034 Stage 0(a) / issue #155: replays the cross-client setup-block vector — this app's
 * first `session-setup` test. A drift between this copy's framing and `apps/web`'s (or the
 * shared `@patches/crypto/setup-block.ts` both now import) fails here, not against a real peer.
 */

...
- **`identity.ts`** — account-root Ed25519 identity; per-device Ed25519 signing key + X25519
  ...
  gap #1: the split identities are no longer independently substitutable); signed monotonic
  device rosters with rollback detection; signed prekey bundles bound to a certificate digest and
  roster digest; a 60-digit, order-independent safety number over sorted root public keys.
...
- **`vectors/`** — deterministic JSON test vectors generated by this implementation from fixed
  ...
  test run so an unintentional protocol regression fails CI.

 ...
 *
 * The composite of `key` + `window_start` uniquely identifies a bucket (both are
 * `@PrimaryColumn` — `rateLimitBucketRepo.increment` looks buckets up by exactly this pair).
 * A single key can have multiple buckets over time as windows expire. The TTL cleanup job
 * removes buckets where `window_end < NOW()`.
 */

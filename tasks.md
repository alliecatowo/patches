...
- [ ] A-027 — TUI (§158 "edit profile"): there is no edit-profile path at all — `UpdateProfile` has a proto RPC and a server handler (`apps/server/src/modules/actors/actor.controller.ts:51`) but `grep -rn "updateProfile" apps/tui/src` returns zero hits, so display name/bio can only ever be set at `patches register` time (`apps/tui/src/cli/register.ts:128`). Done = a profile-edit surface (e.g. `e` on your own `ProfileScreen`, or `patches profile edit`) covering display name + bio + nameplate, with an `apps/tui/test/harness.tsx` snapshot test.
- [ ] A-028 — TUI (§158 "verify email"): `VerifyEmail`/`ResendVerification` have server handlers (`apps/server/src/modules/auth/auth.controller.ts:92,97`) but zero callers in `apps/tui/src` — a user who receives a verification mail has no client command to redeem the code. Done = `patches verify <code>` (+ a resend path) in `apps/tui/src/cli/`, dispatched from `apps/tui/src/cli.tsx`, with a unit test.
...
- [ ] A-032 — layering (§128–129: a controller is a transport adapter, logic lives in an application service): `apps/server/src/modules/federation/http/actor.controller.ts:47–68` injects the `DataSource` and builds TypeORM query-builder / `findOne` queries against `Page` and `PageRevision` inline in the `GET /users/:handle/page` handler. Done = the lookup moves into `PagesService` (or `ActorDocumentService`), `@InjectDataSource`/`typeorm`/`@patches/database` imports are dropped from the controller, and the controller only negotiates content type and serializes.
...
- [ ] A-035 — federation (§160 "Update semantics decided", pre-enable gate): `apps/server/src/modules/federation/services/inbox.service.ts:173–183` handles `Follow`/`Undo`/`Accept`/`Create`/`Delete`/`Like` and silently drops `Update`; `docs/architecture/federation.md:230` records the box unchecked but nothing on the board tracked it. Done = an ADR deciding whether `Update(Note)`/`Update(Actor)` are applied, ignored, or rejected, plus the corresponding inbox branch and an integration test.
- [ ] A-036 — federation (§160 "federation telemetry exists", pre-enable gate): no federation counters/metrics exist — `grep -rn "metric\|telemetry" apps/server/src/modules/federation apps/worker/src/jobs` finds only a comment in `peer-rate-limiter.service.ts:16`; `docs/architecture/federation.md:242` records the box unchecked and untracked. Done = structured counters for inbox accept/reject/dedupe/signature-failure and outbound delivery attempt/success/retry/dead-letter, surfaced through `packages/observability`, documented in `docs/operations/federation.md`.

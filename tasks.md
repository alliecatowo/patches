...
- [x] B-028 — proto+server+TUI: discover a remote actor by `acct:` (WebFinger) via gRPC (`SearchActors` for `user@domain` or a `ResolveActor` RPC) so users can follow remote accounts from the TUI _(server+proto half only: `ActorService.ResolveActor(acct)` — calls `RemoteActorService.resolveByAcct` when `FEDERATION_ENABLED`, else `NOT_IMPLEMENTED`; authenticated, rate-limited per caller (`ActorResolveRateLimitService`); unit-tested with a mocked path via the two-node lab's existing coverage. TUI wiring (a "follow by acct" entry point) is explicitly out of scope for this task — another agent owns `apps/tui`)_
